...
PARAMS = dict(
	imageFormat	= ['jpg', 'png', 'jpeg'],                  		# Specify Image format [JPG / JPEG / PNG ], Default will be taken as JPG
	imageDirectory = './',  		# Source Directory where all images for annotation are kept
	annotationDirectory = './',		# Export Directory where annotations will be stored
	classNames = '0 1 2'						# Class Names separated with spaces
)
...
class AnnotationTool():
	def __init__(self, master):
		...
		self.classNames        = []
		self.classContainer    = PARAMS['classNames']
		self.imageFormat       = PARAMS['imageFormat']
		self.sourceImageDir    = PARAMS['imageDirectory']
		self.annotationDir     = PARAMS['annotationDirectory']
		...
		self.addDelete_photo_btn = Button(self.ctrPanel, text='Delete Image', command=self.deleteImage)
		self.addDelete_photo_btn .pack(side=LEFT, padx=30)
	...
	def deleteImage(self):
		self.image = self.imageList[self.cur - 1]
		self.txt = '.' + self.image.split('.')[1] + '.txt'
		os.remove(self.image)
		if os.path.isfile(self.txt):
			os.remove(self.txt)
			print('txt deleted:', self.txt)
		print('image deleted:', self.image)
